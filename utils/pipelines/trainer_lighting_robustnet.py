...
from utils.models.cov_settings import CovMatrix_IRW



class PLTRobustNet(pl.core.LightningModule):
    r"""
    ...
    def __init__(self,
        ...
        self.cov_matrix = CovMatrix_IRW(relax_denom=2.0)
    ...
    def init_losses(self):
        ...
        # IRW seems to work worse
        # self.robust_loss = IRWLoss()

        self.robust_loss = IWLoss()
    ...
    def training_step(self, batch, batch_idx):
        ...
        out0, out_in0 = self.model(stensor0, is_seg=False)
        sem_preds0 = out0.F
        ...
        if self.current_epoch >= 5:
            aux_loss0 = torch.tensor(0, dtype=torch.float32)
            num_aux = len(out_in0)
            for out_in_tmp0 in out_in0:
                eye, mask_matrix, margin, num_remove_cov = self.cov_matrix(out_in_tmp0.F)
                aux_loss0 += self.robust_loss(out_in_tmp0.F, eye, mask_matrix, margin, num_remove_cov).cpu() / num_aux
        else:
            aux_loss0 = torch.tensor(0, dtype=torch.float32)
        ...
        if self.num_source_domains > 1:
            ...
            out1, out_in1 = self.model(stensor1, is_seg=False)
            sem_preds1 = out1.F
            sem_labels1 = batch['source_sem_labels1'].long()
            sem_loss1 = self.sem_criterion(sem_preds1, sem_labels1).cpu()

            if self.current_epoch >= 5:
                aux_loss1 = torch.tensor(0, dtype=torch.float32)
                num_aux = len(out_in1)
                for out_in_tmp1 in out_in1:
                    eye, mask_matrix, margin, num_remove_cov = self.cov_matrix(out_in_tmp1.F)
                    aux_loss1 += self.robust_loss(out_in_tmp1.F, eye, mask_matrix, margin, num_remove_cov).cpu() / num_aux
            else:
                aux_loss1 = torch.tensor(0, dtype=torch.float32)
        else:
            sem_loss1 = torch.tensor(0, dtype=torch.float32)
            aux_loss1 = torch.tensor(0, dtype=torch.float32)

        total_loss = self.source_weights[0] * sem_loss0 + self.source_weights[1] * sem_loss1
        total_loss = total_loss + 0.5 * (aux_loss0 + aux_loss1)
        ...
        results_dict[f'training/{source0}/aux_loss0'] = aux_loss0.item()
        ...
        if self.num_source_domains > 1:
            ...
            present_labels = present_labels[present_labels != self.ignore_label]
            present_names = self.training_dataset.class2names[present_labels+1].tolist()
            present_names = [os.path.join('training', source1, p + '_iou') for p in present_names]
            results_dict1 = dict(zip(present_names, iou_tmp1.tolist()))
            results_dict1[f'training/{source1}/sem_loss1'] = sem_loss1.item()
            results_dict[f'training/{source1}/aux_loss1'] = aux_loss1.item()
            results_dict1[f'training/{source1}/source_iou1'] = np.mean(iou_tmp1[present_labels])
